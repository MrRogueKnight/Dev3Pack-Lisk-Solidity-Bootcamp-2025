...
const nftCollection = [
  ...
  // Keeping only a few for brevity
];

// We'll get maxSupply from the contract, but provide a fallback
...
export default function NFTPage() {
  ...
  // Get total minted from contract (using totalCounter)
  const { data: totalMintedData } = useScaffoldContractRead({
    contractName: "YourContract",
  ...
  const { data: maxSupplyData } = useScaffoldContractRead({
    contractName: "YourContract",
  ...
  const currentMaxSupply = maxSupplyData ? Number(maxSupplyData) : DEFAULT_MAX_SUPPLY;
  const remainingSupply = Math.max(0, currentMaxSupply - totalMinted);
            ...
            <div className="w-full lg:w-1/3">
              ...
              <div className="mt-6 bg-base-100 p-4 rounded-lg">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-md">
                    <span className="font-semibold">Total Minted:</span> {totalMinted} / {currentMaxSupply}
                  </p>
                  <p className="text-md">
                    <span className="font-semibold">Remaining:</span> {remainingSupply} {isSoldOut ? " (Sold Out!)" : ""}
                  </p>
                  {error && (
              ...
              <div className="mt-4">
                <button
                  ...
                  onClick={handleMint}
                  disabled={isMinting || isMining || hasMinted || isSoldOut}
                >
                  {isMinting || isMining
                    ? "Minting..."
                    : hasMinted
